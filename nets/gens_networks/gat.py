    ...
    def __init__(
        ...
        **kwargs
        ...
        if isinstance(in_channels, int):
            self.temp_weight = torch.nn.Linear(
...
class GensGAT1(nn.Module):
    ...
    def forward(self, x, adj, edge_weight=None):
        ...
        edge_index = adj
        x, edge_index = self.conv1(
            x, edge_index, is_add_self_loops=self.is_add_self_loops
        )
        x = F.relu(x)
...
class GensGAT2(nn.Module):
    def __init__(self, nfeat, nhid, nclass, dropout, nlayer=2):
        super(GensGAT2, self).__init__()
    ...
    def forward(self, x, adj, edge_weight=None):
        ...
        x, edge_index = self.conv2(
            x, edge_index, is_add_self_loops=self.is_add_self_loops
        )
        return x


class GensGATX(nn.Module):
    def __init__(self, nfeat, nhid, nclass, dropout, nlayer=3):
        super(GensGATX, self).__init__()
        ...
        self.convx = nn.ModuleList(
            [GATConv(nhid, head_dim, heads=num_head) for _ in range(nlayer - 2)]
        )
        self.dropout_p = dropout
        self.is_add_self_loops = True
    ...
    def forward(self, x, adj, edge_weight=None):
        ...
        x = F.dropout(x, p=self.dropout_p, training=self.training)
        x, edge_index = self.conv2(
            x, edge_index, edge_weight, is_add_self_loops=self.is_add_self_loops
        )
...
def create_gat(nfeat, nhid, nclass, dropout, nlayer):
    if nlayer == 1:
        model = GensGAT1(nfeat, nhid, nclass, dropout, nlayer)
    elif nlayer == 2:
        model = GensGAT2(nfeat, nhid, nclass, dropout, nlayer)
    else:
        model = GensGATX(nfeat, nhid, nclass, dropout, nlayer)
    return model
