...
from toba import BaseGraphAugmenter, DummyAugmenter
...
class NodeClassificationTrainer:
    ...
    def get_validation_score(self, eval_results: dict):
        ...
        return np.mean(
            [
                ...
                for metric_name in ["acc", "macro-f1"]
            ]
        )
    ...
    def verbose(
        ...
        if runtime:
            ...
            if self.augmenter:
                log += f"| upd/aug time: {runtime_info['update_time(ms)']:.2f}/{runtime_info['aug_time(ms)']:.2f}ms "
